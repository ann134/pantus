...
public class GLView extends GLSurfaceView {
    ...
    private OnModelDetectListener detectListener;

    public void loadModel(int pos, int subPos){
        synchronized (helloAR){
            if(helloAR != null){
                helloAR.loadModel(pos, subPos);
            }
        }
    }

    public void setDetectListener(OnModelDetectListener detectListener) {
        this.detectListener = detectListener;
        helloAR.setModelDetectListener(detectListener);
    }
...
//        setEGLConfigChooser(new ConfigChooser());
        ...
        helloAR = new HelloAR(context, detectListener);

        this.setRenderer(new GLSurfaceView.Renderer() {
            @Override
            public void onSurfaceCreated(GL10 gl, EGLConfig config) {
                synchronized (helloAR) {
                    helloAR.setEglConfig(config);
                    helloAR.initGL(gl, config);
                }
            }
